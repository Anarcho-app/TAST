...
2. **Strict Layer Separation**
   ...
   - `artifacts/` — Downstream synthesis artifacts built on the surviving floor (outputs, not inputs; never feed the likelihood table or move a posterior).
...
## Current Expansion Status (2026-08-04)

- New `artifacts/` layer: downstream syntheses (outputs, not inputs) — Artifact 01 (*Continuous Harm, Declaration Legitimacy, and Collective Cost-Benefit Framework*) + parallel staffer brief, legislative-ready per Checklist v1.0
- `surviving/observable_facts.md` resynchronized to the derived-confidence function (finding 44); parity now gated
- Three artifact-surface gates added: observable-facts parity, artifact grounding, NPV arithmetic
- Ten artifact observables flagged as unregistered registry candidates (see `artifacts/01_continuous_harm_declaration_cba.md` Appendix A)
...
├── artifacts/                # downstream synthesis artifacts (outputs, not inputs)
└── notebooks/

<!-- BEGIN TAST GENERATED HEADER - DO NOT EDIT THIS FILE DIRECTLY. 
  Edit the template: templates/adversarial_H1_likelihoods.md.tmpl
...
<!-- no-exec: full-scale likelihood-uncertainty run; reduced form below is what CI executes -->
```bash
...
CI-affordable form (same paths, smaller N):

```bash
python -m model.bayesian_core --reliability 1.0 --streams model/evidence_streams_full_charitable.csv --lik-uncertainty 200 --kappa 8 --seed 42
```
...
## Reproducibility caveat (2026-07-24)

UNREPRODUCIBLE as of 2026-07-24: the "Measured results" and "Stream 29 effect" sections above report measured effects for Stream 29 (Steckel anthropometric signature) and reference Stream 28. Neither stream identifier is present in any committed CSV (`evidence_streams.csv`, `_adversarial`, `_adversarial_H4`, `_full_charitable` all carry ids 1–27, 30). The reported H1/H5 deltas therefore cannot be reproduced from the current tables.

Restoring the rows requires the original likelihood cells, which are not in the repository. Rather than fabricate cells (which would violate the project's core discipline), the citing sections are retained as narrative with this marker. To reverse: add Stream 28/29 rows to the relevant CSVs and remove this marker. Tracked by `check_documented_commands.py --check-streams`.

...
| 44 | `surviving/observable_facts.md` drifted from the derived-confidence function: 21 of 26 values were stale pre-v5.13 hand-assigned numbers (anti-literacy listed 0.92 vs derived 0.798; genealogical termination 0.88 vs 0.807), six ≥0.85-qualifying facts were absent, and the legislative-facing artifacts cited the stale values as "derived confidence"; separately, two 50-year NPV cells failed annuity-factor recomputation (optimistic +6.5 vs computed +6.43 T; inaction −15.5 vs −15.44 T at 3% real) | Sisyphus (pan-thesis full audit 2026-08-04, epenthesis pass) | md rebuilt from `derive_confidence.py` output (19 high-confidence + 10 below-threshold + 5 unscored context entries); legislative anchor tables corrected to function output (three decimals); NPV cells corrected; three gates added — `check_observable_facts_parity.py`, `check_artifact_grounding.py`, `check_npv_arithmetic.py`. **Closed.** |

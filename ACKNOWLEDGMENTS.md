...
- **Claude Opus 4.8 (sixth pass, 2026-07-24)**: Found that the newly added `--streams` flag printed a success message while silently loading the original table (default-argument binding at definition time + broken global rebind). Also noted that DECISIONS.md had marked items closed that were not yet fixed in the tree, and corrected the full-charitable claim: H1 median stays below prior, but the κ=8 upper tail reaches ~11.8 %. Emphasized that the accumulated adversarial-review credit is for code/doc consistency, not substantive validation of the demographic likelihood values.
